...
class EdgeConnect():
    def __init__(self, config):
        ...
        self.psnr = PSNR(255.0).to(config.DEVICE)#峰值信号比，接近40表示图像很好
        self.edgeacc = EdgeAccuracy(config.EDGE_THRESHOLD).to(config.DEVICE)

        # test mode
        if self.config.MODE == 2:
            print(config.TEST_FLIST)
            self.test_dataset = Dataset(config, config.TEST_FLIST, config.TEST_EDGE_FLIST, config.TEST_MASK_FLIST, augment=False, training=False,is_crop=True)
        else:
            self.train_dataset = Dataset(config, config.TRAIN_FLIST, config.TRAIN_EDGE_FLIST, config.TRAIN_MASK_FLIST, augment=True, training=True,is_crop=True)
    ...
    def train(self):
        ...
        while(keep_training):
            ...
            for items in train_loader:
                ...
                if iteration >= max_iteration+200000:
                    keep_training = False
                    break
    ...
    def test(self,save_path='./result'):
        self.edge_model.eval()
        self.inpaint_model.eval()
        model = self.config.MODEL
        ...
        for items in test_loader:
            ...
            images, images_gray, edges, masks = self.cuda(*items[0])
            shape = items[1]
            ...
            # path = os.path.join('./result/2_sub', name)
            # imsave(output, path,is_reshape=True,reshape=shape)

            if self.debug:
                self.save_my_result(masks,edges,images,name,outputs,shape,save_path)

        print('\nEnd test....')
    def save_my_result(self,masks,edges,images,name,outputs,shape,save_path):
        ww_masks = torch.zeros_like(masks).to(self.config.DEVICE)
        ww_masked = masks - ww_masks
        edges = self.postprocess(1 - edges)[0]
        masked = self.postprocess(images * (1 - ww_masked) + ww_masked)[0]
        fname, fext = name.split('.')

        gt_res = self.postprocess((outputs * ww_masked) + images * (1 - ww_masked))[0]
        outputs = self.postprocess(outputs)[0]
        imsave(gt_res, os.path.join(save_path, fname + '_gt_res.' + 'png'), is_reshape=True, reshape=shape)

        #产生其他结果
        # imsave(outputs, os.path.join('./result', fname + '_res.' + 'png'), is_reshape=True, reshape=shape)
        # imsave(edges, os.path.join('./result', fname + '_edge.' + 'png'))
        # imsave(masked, os.path.join('./result', fname + '_masked.' + 'png'), is_reshape=True, reshape=shape)
